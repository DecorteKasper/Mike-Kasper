<template>
  <div class="flex flex-col">
    ...
    <form @submit.prevent="handleResetPassword" class="w-full mt-10 shadow-cardShadow rounded-cardRadius px-28 py-12">
      <h1 class="text-xl mb-4 font-lato font-bold">Wachtwoord vergeten?</h1>
      <p class="text-xs font-lato text-dark_grey2 mb-4">
        We mailen je een link om je wachtwoord te resetten.
      </p>
...
import type PrimaryButtonVue from '@/components/generic/PrimaryButton.vue'
  ...
  setup() {
    ...
    const rules = computed(() => {
      return {
        email: {
          ...
          email: helpers.withMessage('Vul een geldig email in', email),
        }
      };
    })

    const v$ = useVuelidate(rules, ResetCredentials);
    ...
    const handleResetPassword = () => {
      ...
      if (v$.value.$invalid) {
        ...
        resetPassword(ResetCredentials.email).then(() => {
